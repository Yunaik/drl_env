...
# Relative range.
LARKAGO_BASE_MASS_ERROR_RANGE = (-0.2, 0.2)  # 0.2 means 20%
LARKAGO_LEG_MASS_ERROR_RANGE = (-0.2, 0.2)  # 0.2 means 20%
# Absolute range.
BATTERY_VOLTAGE_RANGE = (14.8, 16.8)  # Unit: Volt
MOTOR_VISCOUS_DAMPING_RANGE = (0, 0.01)  # Unit: N*m*s/rad (torque/angular vel)
LARKAGO_LEG_FRICTION = (0.8, 1.5)  # Unit: dimensionless


class LaikagoEnvRandomizer(env_randomizer_base.EnvRandomizerBase):
  """A randomizer that change the minitaur_gym_env during every reset."""

  def __init__(self,
               minitaur_base_mass_err_range=LARKAGO_BASE_MASS_ERROR_RANGE,
               minitaur_leg_mass_err_range=LARKAGO_LEG_MASS_ERROR_RANGE,
  ...
  def randomize_env(self, env):
    self._randomize_minitaur(env.laikago)
  ...
  def _randomize_minitaur(self, minitaur):
    ...
    Args:
      minitaur: the Laikago instance in minitaur_gym_env environment.
    """
    ...
    randomized_foot_friction = random.uniform(LARKAGO_LEG_FRICTION[0],
                                              LARKAGO_LEG_FRICTION[1])
