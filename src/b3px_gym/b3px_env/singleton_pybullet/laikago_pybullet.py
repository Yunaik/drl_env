...
class Laikago(object):
  """The laikago class that simulates a quadruped robot from Ghost Robotics.
  ...
  def Reset(self, reload_urdf=True):
    """Reset the laikago to its initial states.

    Args:
      reload_urdf: Whether to reload the urdf file. If not, Reset() just place
        the laikago back to its starting position.
    """
    if reload_urdf:
      if self._self_collision_enabled:
        self.quadruped = self._pybullet_client.loadURDF(
            "%s/laikago/laikago.urdf" % self._urdf_root,
            INIT_POSITION, INIT_ORIENTATION,
            flags=self._pybullet_client.URDF_USE_SELF_COLLISION)
      else:
        self.quadruped = self._pybullet_client.loadURDF(
            "%s/laikago/laikago.urdf" % self._urdf_root, INIT_POSITION, INIT_ORIENTATION)
  ...
  def _ResetPoseForLeg(self, leg_id, add_constraint):
    ...
    self._pybullet_client.resetJointState(
        self.quadruped,
        self._joint_name_to_id[leg_position + "_hip_motor_2_chassis_joint"],
        # self._joint_name_to_id["motor_" + leg_position + "L_joint"],
        self._motor_direction[3 * leg_id] * 0.035,
        targetVelocity=0)
    self._pybullet_client.resetJointState(
        self.quadruped,
        self._joint_name_to_id[leg_position + "_upper_leg_2_hip_motor_joint"],
        # self._joint_name_to_id["knee_" + leg_position + "L_link"],
        self._motor_direction[3 * leg_id] * 0.05,
        targetVelocity=0)
    self._pybullet_client.resetJointState(
        self.quadruped,
        self._joint_name_to_id[leg_position + "_lower_leg_2_upper_leg_joint"],
        self._motor_direction[3 * leg_id + 1] * 0.5,
        targetVelocity=0)
    ...
    # if self._accurate_motor_model_enabled or self._pd_control_enabled:
      ...
      # self._pybullet_client.setJointMotorControl2(
      #     bodyIndex=self.quadruped,
      #     jointIndex=(self._joint_name_to_id[leg_position + "lower_leg_2_upper_leg_joint"]),
  ...
  def GetBasePosition(self):
    """Get the position of laikago's base.

    Returns:
      The position of laikago's base.
    """
    position, _ = (
  ...
  def GetJointsPosition(self):
      jInfo = self._pybullet_client.getJointStates(self.quadruped, list(self._joint_name_to_id.values()))
      jPos = [x[0] for x in jInfo]

  def GetBaseOrientation(self):
    """Get the orientation of laikago's base, represented as quaternion.

    Returns:
      The orientation of laikago's base.
    """
    _, orientation = (
        self._pybullet_client.getBasePositionAndOrientation(self.quadruped))
    return orientation
  ...
  def GetObservation(self):
    """Get the observations of laikago.
    ...
    observation.append(self.GetJointsPosition())
    # return self.GetJointsPosition()
    # observation.extend(self.GetJointsPosition())
    return observation
