...
def show(args):
	...
	import joblib
	# params = joblib.load('../data/report-data/laikago/cpu_policy.pkl')
	# policy = params['trainer/policy/']

	policy = joblib.load('../data/report-data/laikago/1.pkl')#cpu_policy.pkl')
	...
	while True:
		...
		while True:
			...
			if np.any(env._env_step_counter == 125):
				break

	if cfg['is_render']:
		imageio.mimwrite(cfg['video_name'].format(datetime.datetime.now()), np.asarray(imgs), fps = 40.0)
...
if __name__ == '__main__':
	...
	parser.add_argument('--seed', type=int, default=666666, metavar='N',
	                    help='random seed (default: 666666)')
