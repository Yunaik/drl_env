...
class Laikago(object):
    ...
    def __init__(self, client ,enable_pd = False, kp=10, kd=0.1, sim_freq = 1000, ctrl_freq = 500, cmd_freq=25, motor_vel_limit = 10, motor_torq_limix = 40,urdf='/laikago.urdf', urdf_root=None, fixed = False, usePhysx = False, plane = -1, pos = None):
        ...
        # Get Uid
        if fixed:
            self.uid = self._p.loadURDF(os.path.join(urdf_root, urdf), pos, self.INIT_POS, useFixedBase=True)
        else:
    ...
    # def Apply(self, act_cmd, record_joints = False):
    #     """
    ...
    #             q = self.GetMotorAng()
    #             qdot = self.GetMotroVel()
    #             torq = self.GetMotorTorq()

    #             print("Ang:", q * 180 / np.pi)
    #             print("Vel:", qdot)
    #             print("torq : ", torq)

    #             torque_cmd = -self.kp * (q - act_cmd) - self.kd * qdot
    #             print("torq cmd [before clip] : ",torque_cmd)

    #             torque_cmd = np.clip(torque_cmd, -MOTORS_TORQ_LIMIT, MOTORS_TORQ_LIMIT)
    #             print("torq cmd [after clip] : ", torque_cmd)

    #             self._observed_motor_torques = torque_cmd
    #             self._applied_motor_torq = np.multiply(self._observed_motor_torques, MOTOR_DIRECTION)


    #             self.SetMotorVels(self._applied_motor_torq)
    ...
    # def Apply_Pos(self, act_cmd, clip = True, record_joints = False):
    #     """
    #     Robot Simulation Env, every cmd exec need to run (sim_freq / cmd_freq) sim_tims steps;
    ...
    #         # cur_cmd_js.append(self.GetPosObs())
    #     if clip:
    #         act_cmd = np.clip(act_cmd, JOINT_LIMIT_LOWER, JOINT_LIMIT_UPPER)
