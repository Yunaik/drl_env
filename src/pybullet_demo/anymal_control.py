...
class Anymal:
    def __init__(self, usePhysX, uuid, basePos = [0, 0, 0]):
        ...
        self.uId = p.loadURDF("urdf/anymal_bedi_urdf/anymal.urdf", basePos, [0, 0.707, 0.707, 0])
...
def env_init(bCfg):
    ...
    offset = u*dist/2
    return [Anymal(bCfg['physx'], bCfg['uuid'] + str(i), basePos = [ i // u * dist-offset, i % u * dist-offset, 4.7]) for i in range(ins_num)]
