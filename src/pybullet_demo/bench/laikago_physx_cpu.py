...
class Laikago:
    def __init__(self, usePhysX, uuid, basePos = [0, 0, 0]):
        ...
        # self.uId = p.loadURDF('../../urdf/anymal_bedi_urdf/anymal.urdf', basePos)
        self.uId = p.loadURDF('../../urdf/laikago_description/laikago_gpu.urdf', basePos)
...
def env_init(bCfg):
    ...
    p.loadURDF("plane100.urdf", useMaximalCoordinates=True)  # useMaximalCoordinates)

    ins_num = bCfg['instance']
    u = np.floor(np.sqrt(ins_num)) + 1
    dist = bCfg['instance_dist']
    offset = u*dist/2
    return [Laikago(bCfg['physx'], bCfg['uuid'] + str(i), basePos = [ i // u * dist-offset, i % u * dist-offset, 0.5]) for i in range(ins_num)]
...
if __name__ == '__main__':
    ...
    defaultBenchCfg = {
        ...
        'need_log': False,
        'profile': False,
        'profile_name': 'default_profile.json'
    }
